...
esp_err_t NfcDriver::Init() {
    ...
    // 完全对齐参考工程 NFC_init 实现
    uart_config_t uart_config = {
        .baud_rate = (int)config_.baudrate,  // 使用配置的波特率
    ...
    // 尝试安装 UART 驱动，如果已安装则跳过（避免重复初始化崩溃）
    esp_err_t ret = uart_driver_install(config_.uart_port, 1024, 1024, 0, NULL, 0);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "UART driver installed successfully");
    } else if (ret == ESP_FAIL) {
        // UART 驱动已安装，这是正常情况（其他模块可能已初始化）
        ESP_LOGW(TAG, "UART driver already installed on port %d, reusing existing driver", config_.uart_port);
    } else {
        ESP_LOGE(TAG, "Failed to install UART driver: %s", esp_err_to_name(ret));
        return ret;
    }
    
    // 配置 UART 参数（即使驱动已安装也可以重新配置）
    ...
    initialized_ = true;
    ESP_LOGI(TAG, "NFC module UART initialized.");
    return ESP_OK;
}
...
esp_err_t NfcDriver::SendPollingCommand() {
    ...
    // 打印发送的命令
    ESP_LOGI(TAG, "Sending polling cmd (%d bytes): 43 4D 74 02 03 00 00 00 00 00", 
             (int)sizeof(NFC_POLLING_CMD));

    // 参考工程：直接发送命令，不做 flush
    ...
    if (written != sizeof(NFC_POLLING_CMD)) {
        ESP_LOGE(TAG, "Failed to send polling command, written=%d", written);
        return ESP_FAIL;
    }

    // 等待发送完成（参考工程使用 20ms）
    uart_wait_tx_done(config_.uart_port, pdMS_TO_TICKS(20));
    ESP_LOGI(TAG, "Polling cmd sent, waiting for response...");
...
bool NfcDriver::ReadCard(nfc_card_info_t& card_info, uint32_t timeout_ms) {
    ...
    // 参考工程使用 100ms 超时
    int len = uart_read_bytes(config_.uart_port, rx_buffer, sizeof(rx_buffer), pdMS_TO_TICKS(100));

    if (len <= 0) {
        ESP_LOGI(TAG, "No response (len=%d, UART%d, TX=%d, RX=%d)", 
                 len, config_.uart_port, config_.tx_pin, config_.rx_pin);
        return false;
    }

    // 调试日志：打印响应数据
    ESP_LOGI(TAG, "Response received, len=%d bytes:", len);
...
void NfcDriver::ContinuousTaskFunc(void* arg) {
    ...
    while (driver->continuous_running_.load()) {
        ...
        if (driver->PollAndRead(card_info, 150)) {
            // 检测到卡片
