...
BluetoothService::BluetoothService() 
    : initialized_(false), connected_(false), authenticated_(false), 
      ...
      message_queue_(nullptr), process_task_(nullptr), process_task_running_(false) {
    g_instance = this;
...
bool BluetoothService::Initialize(const std::string& device_name) {
    ...
    // ═══════════════════════════════════════════════════════════════
    // 创建异步消息处理队列和任务
    // 解决 BLE 回调中执行复杂逻辑导致 nimble_host 栈溢出的问题
    // ═══════════════════════════════════════════════════════════════
    ESP_LOGI(TAG, "📬 创建异步消息处理队列...");
    
    // 创建消息队列（存储字符串指针）
    message_queue_ = xQueueCreate(MESSAGE_QUEUE_SIZE, sizeof(char*));
    ...
    ESP_LOGI(TAG, "✅ 异步消息处理已启用（队列大小: %d, 任务栈: %d 字节）",
             MESSAGE_QUEUE_SIZE, PROCESS_TASK_STACK);
...
void BluetoothService::Deinitialize() {
    ...
    // 步骤0: 停止异步消息处理任务
    if (process_task_ != nullptr) {
        ESP_LOGI(TAG, "📬 停止消息处理任务...");
        process_task_running_ = false;
        
        // 等待任务退出（最多 500ms）
        for (int i = 0; i < 50 && eTaskGetState(process_task_) != eDeleted; i++) {
            vTaskDelay(pdMS_TO_TICKS(10));
        }
        process_task_ = nullptr;
...
void BluetoothService::ProcessReceivedData(const std::string& data) {
    ...
    while (newline_pos != std::string::npos) {
        ...
        // 将消息放入队列异步处理（避免在 nimble_host 栈上执行复杂逻辑）
        EnqueueMessage(complete_message);
...
void BluetoothService::EnqueueMessage(const std::string& message) {
    if (message_queue_ == nullptr) {
        ESP_LOGW(TAG, "⚠️ 消息队列未创建，直接调用回调（可能导致栈溢出）");
        if (data_received_callback_) {
            data_received_callback_(message);
        }
        return;
    }
    ...
    if (message.length() > MAX_MESSAGE_LENGTH) {
        ESP_LOGW(TAG, "⚠️ 消息过长（%d > %d），截断处理", 
                 message.length(), MAX_MESSAGE_LENGTH);
    }
    
    // 分配内存并复制消息（处理任务负责释放）
    size_t copy_len = (message.length() < MAX_MESSAGE_LENGTH) ? message.length() : MAX_MESSAGE_LENGTH;
    char* msg_copy = (char*)malloc(copy_len + 1);
    if (msg_copy == nullptr) {
        ESP_LOGE(TAG, "❌ 分配消息内存失败");
        return;
    }
    
    memcpy(msg_copy, message.c_str(), copy_len);
    msg_copy[copy_len] = '\0';
    
    // 放入队列（不等待，如果队列满则丢弃）
    if (xQueueSend(message_queue_, &msg_copy, 0) != pdTRUE) {
        ESP_LOGW(TAG, "⚠️ 消息队列已满，丢弃消息");
        free(msg_copy);
    } else {
        ESP_LOGD(TAG, "📬 消息已入队（长度: %d）", copy_len);
    }
}

void BluetoothService::ProcessTaskEntry(void* arg) {
    BluetoothService* self = static_cast<BluetoothService*>(arg);
    self->ProcessTaskLoop();
}

void BluetoothService::ProcessTaskLoop() {
    ESP_LOGI(TAG, "📬 消息处理任务已启动");
    
    char* msg_ptr = nullptr;
    
    while (process_task_running_) {
        // 等待队列中的消息（最多等待 100ms，然后检查退出标志）
        if (xQueueReceive(message_queue_, &msg_ptr, pdMS_TO_TICKS(100)) == pdTRUE) {
            if (msg_ptr != nullptr) {
                ESP_LOGI(TAG, "📬 处理队列消息（在专用任务中，栈充足）");
    ...
    // 清空队列中剩余的消息
    while (xQueueReceive(message_queue_, &msg_ptr, 0) == pdTRUE) {
        if (msg_ptr != nullptr) {
            free(msg_ptr);
        }
    }
    
    ESP_LOGI(TAG, "📬 消息处理任务已退出");
    vTaskDelete(nullptr);
}
