...
bool AnimationStateManager::SetDeviceStateWithPriority(DeviceState state, Priority priority, bool force) {
    ...
    // 无论动画是否切换成功，都记录当前设备状态
    // 这样情感动画超时后可以恢复到正确的设备状态
    current_device_state_ = state;
...
bool AnimationStateManager::RestoreToCurrentDeviceState() {
    // 使用当前设备状态（而不是历史栈中保存的状态）
    if (current_device_state_ == DeviceState::Unknown) {
        ESP_LOGW(TAG, "No current device state to restore");
        return RestorePreviousState();  // 回退到旧逻辑
    }
    
    auto it = device_state_map_.find(current_device_state_);
    if (it == device_state_map_.end()) {
        ESP_LOGW(TAG, "Current device state not registered: %d", static_cast<int>(current_device_state_));
        return RestorePreviousState();  // 回退到旧逻辑
    }
    
    ESP_LOGI(TAG, "Restoring to current device state: %s", GetDeviceStateName(current_device_state_));
