...
def process_aaf_animations(aaf_animations_dir, assets_dir):
    """Process AAF animation files (.aaf)"""
    if not aaf_animations_dir:
        return []
    
    if not os.path.exists(aaf_animations_dir):
        print(f"Info: AAF animations directory not found: {aaf_animations_dir}")
        print(f"  Skipping AAF animations (this is normal if you haven't added animation files yet)")
        return []
    
    aaf_files_list = []
    
    # Copy each .aaf file from animations directory
    for file in os.listdir(aaf_animations_dir):
        if file.lower().endswith('.aaf'):
            src_file = os.path.join(aaf_animations_dir, file)
            dst_file = os.path.join(assets_dir, file)
            if copy_file(src_file, dst_file):
                # Get file size for reporting
                file_size = os.path.getsize(src_file)
                aaf_files_list.append({
                    "name": file,
                    "size": file_size
                })
    
    if aaf_files_list:
        total_size = sum(item['size'] for item in aaf_files_list)
        print(f"✅ Processed {len(aaf_files_list)} AAF animation files ({total_size / 1024:.1f} KB total)")
        for item in aaf_files_list:
            print(f"   - {item['name']}: {item['size'] / 1024:.1f} KB")
    else:
        print(f"Info: No AAF animation files found in: {aaf_animations_dir}")
    
    return [item['name'] for item in aaf_files_list]


def generate_index_json(assets_dir, srmodels, text_font, emoji_collection, extra_files=None, aaf_animations=None, multinet_model_info=None):
    ...
    if aaf_animations:
        index_data["aaf_animations"] = aaf_animations
...
def build_assets_integrated(wakenet_model_paths, multinet_model_paths, extra_model_paths, text_font_path, emoji_collection_path, extra_files_path, aaf_animations_path, output_path, multinet_model_info=None, esp_sr_model_path=None):
    ...
    try:
        ...
        aaf_animations = process_aaf_animations(aaf_animations_path, assets_dir) if aaf_animations_path else None
        
        # Generate index.json
        generate_index_json(assets_dir, srmodels, text_font, emoji_collection, extra_files, aaf_animations, multinet_model_info)
...
def main():
    ...
    parser.add_argument('--aaf_animations', help='Path to AAF animation files directory (e.g., main/assets/animations)')
    ...
    # Get AAF animations path if provided
    aaf_animations_path = args.aaf_animations
    if not aaf_animations_path:
        # Default to main/assets/animations
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir)
        aaf_animations_path = os.path.join(project_root, "main", "assets", "animations")
    ...
    if not wakenet_model_paths and not multinet_model_paths and not text_font_path and not emoji_collection_path and not extra_files_path and not aaf_animations_path and not multinet_model_info:
        print("Warning: No assets to build (no SR models, text font, emoji collection, extra files, AAF animations, or custom wake word)")
